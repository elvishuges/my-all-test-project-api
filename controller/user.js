...
const registration = [
  check("fullName")
    .matches(/^[a-zA-Z ]+$/)
    .withMessage("Only alphabets and at least one space are allowed"),
...
const getUsers = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: true, message: "Data Fetched Successfully", users });
  } catch (error) {
    console.error(error);
    res.status(500).json({ status: false, message: "Internal Server Error" });
  }
...
export { registration, login, changePassword, getUsers, removeUser };
